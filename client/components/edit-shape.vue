...
export default {
    ...
    methods: {
        ...
        handleMouseDownOnElement (e) {
            ...
            let startTop = isNaN(pos.top) ? 0 : pos.top
            let startLeft = isNaN(pos.left) ? 0 : pos.left
            ...
            let move = moveEvent => {
                ...
                pos.top = Number(Math.floor((currY - startY) / this.projectData.scale) + startTop)
                pos.left = Number(Math.floor((currX - startX) / this.projectData.scale) + startLeft) 
        ...
        handleMouseDownOnPoint (point) {
            ...
            let move = moveEvent => {
                let currX = moveEvent.clientX
                let currY = moveEvent.clientY
                let disY = Math.floor((currY - startY) / this.projectData.scale)
                let disX = Math.floor((currX - startX) / this.projectData.scale)
                let hasT = /t/.test(point)
                let hasB = /b/.test(point)
                let hasL = /l/.test(point)
                let hasR = /r/.test(point)
